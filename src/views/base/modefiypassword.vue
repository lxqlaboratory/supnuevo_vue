...
<script>
  import { modefypassword } from '@/api/student'
  export default {
    ...
    methods: {
      submitForm() {
        modefypassword({ 'oldpassword': this.ruleForm.oldPass, 'newpassword1': this.ruleForm.newPass, 'newpassword2': this.ruleForm.checkPass }).then(res => {
          if (res.code === 0) {
            this.$message({
              type: 'error',
